...
export function applyVisualLibrary(
  ...
  return {
    ...
    background: hit.dna?.imageTreatment || ad.background,
...
export function buildPrompt(
  ...
  return [
    ...
    `7. HUMAN SUBJECT (non-negotiable): at least one photoreal person. Placement: ${dna?.humanPlacement || ad.human.framing}. Role: ${dna?.humanRole || ad.human.role}. Action: ${ad.human.action}. Wardrobe: ${ad.human.wardrobe}. Expression: ${ad.human.expression}. Why they are there: ${ad.human.why}.`,
    input.mainImageUrl
    ...
    "12. MARGINS: keep the same breathing rhythm as the reference. No important element touching the edge.",
    `13. SAFE ZONE: ${dna?.spacing || ad.negative_space}.`,
