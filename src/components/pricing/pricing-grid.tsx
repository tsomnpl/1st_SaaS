        ...
        return (
          <article
            ...
            className={`flex flex-col rounded-[1.6rem] border bg-white p-6 ${
              highlighted
                ? "border-[#20C997] shadow-[0_20px_50px_rgba(32,201,151,0.16)]"
                : "border-slate-200"
            }`}
          >
            <div className="flex items-center justify-between gap-2">
              ...
              {highlighted ? (
                <span className="rounded-full bg-[#DFFAF0] px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide text-[#111827]">
                  Le plus choisi
                </span>
              ) : null}
            </div>
            <h2 className="mt-3 text-2xl font-extrabold tracking-tight text-[#111827]">
              {plan.headline}
            </h2>
            ...
            <ul className="mt-5 flex-1 space-y-2.5 text-sm text-slate-700">
              {plan.features.map((feature) => (
                <li key={feature} className="flex gap-2">
                  <span className="mt-0.5 text-[#20C997]">✓</span>
                  <span>{feature}</span>
                </li>
              ))}
            </ul>
